...
export function ChatBubbleInputWord({ item, isBookmarked, onEnterValue, onChagneText, inputRef, input, isVoiceMode, isLastItem, isSpeaking, speakingList, onIsClickSpeakChange }: ChatBubbleProps) {
	useEffect(() => {
		setIsBookmark(isBookmarked);
	}, [isBookmarked]);
...
export function ChatBubbleInputAll({ item, isBookmarked, onEnterValue, onChagneText, inputRef, input, isVoiceMode, isLastItem, isSpeaking, speakingList, onIsClickSpeakChange }: ChatBubbleProps) {
	useEffect(() => {
		setIsBookmark(isBookmarked);
	}, [isBookmarked]);
...
function wordBundle(text: string) {
	return (
		<Text style={{ backgroundColor: Colors.GRAY08 }}>
			{text}
		</Text>
	)
}

// function wordInputBundle(text: string, inputRef: React.RefObject<TextInput>, input: string, onEnterValue: () => void, onChagneText: (text: string) => void ) {
// 	return (
// 		<TextInput
// 			style={[inputBubbleStyles.input, { color: Colors.WHITE, marginVertical: 4 }]}
// 			ref={inputRef}
